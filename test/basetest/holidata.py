...
from .utils import run_cmd_wait, run_cmd_wait_nofail, which, holidata_binary_location, DEFAULT_EXTENSION_PATH
from .compat import STRING_TYPE


class Holidata(object):
    """Manage a Holidata instance

    A temporary folder is used as data store of holidata.

    A holidata client should not be used after being destroyed.
    """
    DEFAULT_HOLIDATA = holidata_binary_location()

    def __init__(self, holidata=DEFAULT_HOLIDATA):
        """Initialize a holidata (client).
        The program runs in a temporary folder.

        :arg holidata: Holidata binary to use as client (defaults: holidata in PATH)
        """
        self.holidata = holidata

        # Used to specify what command to launch (and to inject faketime)
        self._command = [self.holidata]
    ...
    def __repr__(self):
        txt = super(Holidata, self).__repr__()
        return "{0} running from {1}>".format(txt[:-1], self.datadir)

    def __call__(self, *args, **kwargs):
        "aka t = Holidata() ; t() which is now an alias to t.runSuccess()"
    ...
    def runSuccess(self, args="", input=None, merge_streams=False,
                   timeout=5):
        """Invoke holidata with given arguments and fail if exit code != 0

        Use runError if you want exit_code to be tested automatically and
        *not* fail if program finishes abnormally.

        If you wish to pass instructions to holidata such as confirmations or other
        ...
        timeout = number of seconds the test will wait for every holidata call.
